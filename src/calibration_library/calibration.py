...
def find_best_seafloor_detection(ed_raw, ds_Sv, min_valid_depth_m=10.0):
    ...
    for ch_idx, channel in enumerate(ds_Sv.channel):
        ...
        if n_valid == 0:
            print(f"   X {freq_khz:3d} kHz: NO data")
            continue
        elif n_zeros == n_valid:
            print(f"   X {freq_khz:3d} kHz: ALL zeros")
            continue
        elif n_deep_enough == 0:
            mean_depth = valid_values.mean()
            print(f"   Warning  {freq_khz:3d} kHz: Too shallow (mean={mean_depth:.1f}m)")
            continue
        
        # Score based on: number of valid deep detections, mean depth, consistency
        mean_depth = valid_values[valid_values >= min_valid_depth_m].mean()
        std_depth = valid_values[valid_values >= min_valid_depth_m].std()
        score = n_deep_enough * (1.0 - min(std_depth / mean_depth, 1.0))  # Favor consistency
        
        print(f"   ✓  {freq_khz:3d} kHz: {n_deep_enough}/{n_valid} valid, mean={mean_depth:.1f}m, std={std_depth:.1f}m, score={score:.0f}")
    ...
    print(f"\n Best seafloor detection: {best_freq_khz} kHz (channel {best_channel_idx})")
...
def remove_seafloor_from_mask(ed_raw, ds_Sv, mask, buffer_m=1.0, use_best_detection=True, min_valid_depth_m=10.0):
    """Create mask to exclude data below detected seafloor
    
    Args:
        ed_raw: Original EchoData object with seafloor detection
        ds_Sv: Calibrated Sv dataset from ep.calibrate.compute_Sv()
        buffer_m: Buffer distance in meters above seafloor to maintain
        mask: Boolean array mask to be updated by this function
        use_best_detection: If True, find and use the best seafloor detection for all channels
        min_valid_depth_m: Minimum depth to consider as valid seafloor (default 10m)
    """
    ...
    if use_best_detection:
        # Find the best seafloor detection across all channels
        best_ch_idx, best_freq_khz, best_seafloor = find_best_seafloor_detection(
            ed_raw, ds_Sv, min_valid_depth_m
        )
        print(f"\n Applying {best_freq_khz} kHz seafloor detection to ALL channels with {buffer_m}m buffer...")
        
        # Use the best detection for all channels
        for ch_idx, channel in enumerate(ds_Sv.channel):
            freq_hz = ds_Sv["frequency_nominal"].values[ch_idx]
            freq_khz = int(freq_hz / 1000)
            
            for ping_idx in range(len(ds_Sv.ping_time)):
                try:
                    # Use the best channel's seafloor depth
                    seafloor_at_ping = best_seafloor.isel(ping_time=ping_idx)
                    
                    # Get the range values for this channel and ping
                    ping_ranges = echo_range.isel(channel=ch_idx, ping_time=ping_idx)
                    
                    # Create range mask: True = keep data (above seafloor + buffer)
                    range_mask = ping_ranges <= (seafloor_at_ping - buffer_m)
                    
                    # Apply the mask for this channel and ping
                    mask[ch_idx, ping_idx, :] = range_mask.values & mask[ch_idx, ping_idx, :]
                    
                except (KeyError, IndexError, ValueError) as e:
                    if ping_idx % 100 == 0:
                        print(f"   ⚠️  Issue at channel {ch_idx}, ping {ping_idx}: {e}")
                    continue
...
def mask_frequency_channels(ds_Sv, mask, frequencies_to_mask_khz):
    ...
    for ch_idx, channel in enumerate(ds_Sv.channel):
        ...
        if freq_khz in frequencies_to_mask_khz:
            # Mask entire channel (set all to False = masked)
            mask[ch_idx, :, :] = False
            print(f"   ✓ Masked all data for {freq_khz} kHz")
    
    return mask


def remove_surface_from_mask(ds_Sv, mask, depth_threshold_m):
    """Create mask to exclude data from surface down to specified depth threshold
    
    Args:
        ds_Sv: Calibrated Sv dataset from ep.calibrate.compute_Sv()
        depth_threshold_m: Depth threshold in meters below surface to mask
    
    Returns:
        mask: Boolean array where True = keep data (below threshold), False = mask data (at or above threshold)
    """
    ...
    # Apply surface mask using vectorized operation: True = keep data (below threshold depth)
    # False = mask data (at or above threshold depth, i.e., near surface)
    mask = mask & (echo_range > depth_threshold_m)
    return mask


def apply_mask_to_sv(ds_Sv, mask, fill_value=np.nan):
    """Apply seafloor mask to any Sv dataset
    
    Args:
        ds_Sv: Calibrated Sv dataset from ep.calibrate.compute_Sv()
        mask: The mask to apply
        fill_value: Value to use for masked data (default: np.nan)
    
    Returns:
        ds_Sv_masked: Dataset with seafloor mask applied
    """
